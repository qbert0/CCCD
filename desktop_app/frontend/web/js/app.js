  ...
  const App = {
    ...
    methods: {
      ...
      bindCommonToService(templateValue = state.service_template) {
        ...
        } else if (option.source_role === "new_123_old_456") {
          // Cá nhân -> cá nhân: images 1-3 are the new owner, 4-6 the old
          // one -- swapped from the org->individual naming's own
          // "old first" order by explicit request.
          state.new_owner = common.person123;
          state.customer = common.person456;
          state.new_owner_photo_path = common.photo123Path;
          state.customer_photo_path = common.photo456Path;
        } else {
